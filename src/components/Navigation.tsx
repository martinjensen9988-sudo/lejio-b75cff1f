    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${!isVisible && !isOpen ? "-translate-y-full" : "translate-y-0"} ${isScrolled || isOpen ? "bg-card/95 backdrop-blur-md shadow-soft border-b border-border" : "bg-transparent"}`}>
      <div className="container mx-auto px-4 sm:px-6 py-3 sm:py-4">
        <div className="flex items-center justify-between">
          ...
          <button 
            className="lg:hidden text-foreground p-2.5 rounded-xl hover:bg-muted transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center" 
        ...
        {isOpen && (
          <div className="lg:hidden fixed inset-x-0 top-full bg-card border-b border-border shadow-2xl max-h-[calc(100vh-64px)] overflow-y-auto">
            <div className="px-4 sm:px-6 py-4 flex flex-col gap-1">
              {/* User logged in - show user menu */}
              {user ? (
                <>
                  <div className="flex items-center gap-3 px-4 py-4 rounded-xl bg-muted/50 mb-3">
                  ...
                  <a href="/dashboard" onClick={(e) => { e.preventDefault(); navigate("/dashboard"); setIsOpen(false); }} className="flex items-center gap-3 text-foreground hover:text-primary font-medium py-3.5 px-4 rounded-xl hover:bg-muted transition-colors min-h-[48px]">
                    <User className="w-5 h-5" />
                    Dashboard
                  </a>
                  <a href="/search" onClick={(e) => { e.preventDefault(); navigate("/search"); setIsOpen(false); }} className="flex items-center gap-3 text-muted-foreground hover:text-primary font-medium py-3.5 px-4 rounded-xl hover:bg-muted transition-colors min-h-[48px]">
                    <Search className="w-5 h-5" />
                    Find bil
                  </a>
                  <a href="/my-rentals" onClick={(e) => { e.preventDefault(); navigate("/my-rentals"); setIsOpen(false); }} className="flex items-center gap-3 text-muted-foreground hover:text-primary font-medium py-3.5 px-4 rounded-xl hover:bg-muted transition-colors min-h-[48px]">
                    <FileText className="w-5 h-5" />
                    Mine lejeaftaler
                  </a>
                  <a href="/beskeder" onClick={(e) => { e.preventDefault(); navigate("/beskeder"); setIsOpen(false); }} className="flex items-center gap-3 text-muted-foreground hover:text-primary font-medium py-3.5 px-4 rounded-xl hover:bg-muted transition-colors min-h-[48px]">
                    <MessageCircle className="w-5 h-5" />
                    Beskeder
                  </a>
                  <a href="/settings" onClick={(e) => { e.preventDefault(); navigate("/settings"); setIsOpen(false); }} className="flex items-center gap-3 text-muted-foreground hover:text-primary font-medium py-3.5 px-4 rounded-xl hover:bg-muted transition-colors min-h-[48px]">
                ...
                <>
                  ...
                  <a href="/search" onClick={(e) => { e.preventDefault(); navigate("/search"); setIsOpen(false); }} className="text-muted-foreground hover:text-primary font-medium py-3.5 px-4 rounded-xl hover:bg-muted transition-colors min-h-[48px] flex items-center">Find bil</a>
                  <a href="#features" onClick={() => setIsOpen(false)} className="text-muted-foreground hover:text-primary font-medium py-3.5 px-4 rounded-xl hover:bg-muted transition-colors min-h-[48px] flex items-center">Funktioner</a>
                  <a href="#how-it-works" onClick={() => setIsOpen(false)} className="text-muted-foreground hover:text-primary font-medium py-3.5 px-4 rounded-xl hover:bg-muted transition-colors min-h-[48px] flex items-center">Sådan virker det</a>
                  <a href="#pricing" onClick={() => setIsOpen(false)} className="text-muted-foreground hover:text-primary font-medium py-3.5 px-4 rounded-xl hover:bg-muted transition-colors min-h-[48px] flex items-center">Priser</a>
                  <a href="/faq" onClick={(e) => { e.preventDefault(); navigate("/faq"); setIsOpen(false); }} className="text-muted-foreground hover:text-primary font-medium py-3.5 px-4 rounded-xl hover:bg-muted transition-colors min-h-[48px] flex items-center">FAQ</a>
