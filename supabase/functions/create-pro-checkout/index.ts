...
serve(async (req) => {
  ...
  try {
    ...
    if (profile?.user_type !== 'professionel') {
      throw new Error("Pro subscription is only available for professional users (CVR)");
    }
    ...
    if (customers.data.length > 0) {
      ...
      if (subscriptions.data.length > 0) {
        ...
        throw new Error("Du har allerede et aktivt abonnement. Brug kundeportalen til at ændre dit abonnement.");
      }
    }
